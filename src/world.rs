...
//! All geometry is emitted as `WorldTri` in world space and generated once at startup.
...
const BUILDING_COLORS: [u32; 10] = [
    0xFFCCBB99, 0xFFBBAA88, 0xFFDDCCAA, 0xFFBBBBAA, 0xFFCCBBAA, 0xFFD4C4A8, 0xFFE0D0B0, 0xFFC8B898,
    0xFFCCCCBB, 0xFFBBAAAA,
];
...
pub fn overlaps_building(x: f32, z: f32, margin: f32, buildings: &[Building]) -> bool {
    buildings
        .iter()
        .any(|b| (x - b.x).abs() < b.w * 0.5 + margin && (z - b.z).abs() < b.d * 0.5 + margin)
}
...
pub fn surface_at(x: f32, z: f32, net: &RoadNetwork) -> Surface {
    ...
    match best_tier {
        RoadTier::CarRoad => {
            if best_dist < CAR_ROAD_WIDTH * 0.5 {
                Surface::CarRoad
            } else if best_dist < CAR_ROAD_WIDTH * 0.5 + SIDEWALK_WIDTH {
                Surface::Sidewalk
            } else {
                Surface::Terrain
            }
        ...
        RoadTier::FieldRoad => {
            if best_dist < FIELD_ROAD_WIDTH * 0.5 {
                Surface::FieldRoad
            } else {
                Surface::Terrain
            }
...
fn generate_road_network(rng: &mut Rng, settlement_center: [f32; 2]) -> RoadNetwork {
    ...
    for i in 0..ring1_count {
        let angle = base_angle
            + (i as f32 / ring1_count as f32) * std::f32::consts::TAU
            + rng.range(-0.3, 0.3);
    ...
    for i in 0..ring2_count {
        let angle = base_angle2
            + (i as f32 / ring2_count as f32) * std::f32::consts::TAU
            + rng.range(-0.25, 0.25);
    ...
    for i in 0..edge_count {
        let angle = base_angle3
            + (i as f32 / edge_count as f32) * std::f32::consts::TAU
            + rng.range(-0.3, 0.3);
    ...
    for i in 0..ring1_count {
        ...
        let mut dists: Vec<(f32, usize)> = (0..ring2_count)
            .map(|j| {
    ...
    for _ in 0..field_count {
        ...
        if dist > max_field_len || dist < min_field_len {
            continue;
        }
        let already = segments.iter().any(|s| {
            (s.x0 == nodes[a][0]
...
fn build_road_graph(nodes: &[[f32; 2]], segments: &[RoadSegment]) -> RoadGraph {
    ...
    for (seg_idx, seg) in segments.iter().enumerate() {
        ...
        for (ni, node) in nodes.iter().enumerate() {
            let da =
                (seg.x0 - node[0]) * (seg.x0 - node[0]) + (seg.z0 - node[1]) * (seg.z0 - node[1]);
            let db =
                (seg.x1 - node[0]) * (seg.x1 - node[0]) + (seg.z1 - node[1]) * (seg.z1 - node[1]);
            if da < best_a_dist {
                best_a_dist = da;
                best_a = ni;
            }
        ...
        if !adjacency[best_a]
            .iter()
            .any(|&(n, s, _)| n == best_b && s == seg_idx)
        {
            adjacency[best_a].push((best_b, seg_idx, dist));
        }
        if !adjacency[best_b]
            .iter()
            .any(|&(n, s, _)| n == best_a && s == seg_idx)
        {
...
fn generate_parking_spots(
    ...
    for seg in &net.segments {
        ...
        let Some((perp_x, perp_z, dir_x, dir_z, len)) = segment_perp(seg) else {
            continue;
        };
        if len < 20.0 {
            continue;
        ...
        for side in [-1.0f32, 1.0] {
            ...
            for k in 0..num_spots {
                ...
                if overlaps_building(sx, sz, 1.0, buildings) {
                    continue;
                }

                // Skip if near river (parking + vehicle width margin)
                if near_river(sx, sz, river_segments, 4.0) {
                    continue;
                }
...
fn add_building_foundation(
    ...
    if skirt_h < 0.05 {
        return;
    } // terrain is flat under building, no skirt needed
...
fn erode_terrain(terrain: &mut Terrain, seed: u64, roughness: f32) {
    ...
    for _ in 0..num_drops {
        ...
        for _ in 0..max_lifetime {
            let ix = px as usize;
            let iz = pz as usize;
            if ix < 1 || ix >= grid - 1 || iz < 1 || iz >= grid - 1 {
                break;
            }
            ...
            let nix = nx as usize;
            let niz = nz as usize;
            if nix < 1 || nix >= grid - 1 || niz < 1 || niz >= grid - 1 {
                break;
            }
...
fn generate_terrain_mesh(tris: &mut Vec<WorldTri>, terrain: &Terrain, zone_map: &zone::ZoneMap) {
    ...
    for iz in 0..grid {
        for ix in 0..grid {
            ...
            let cell_hash = (ix as u32).wrapping_mul(73856093) ^ (iz as u32).wrapping_mul(19349663);
...
fn generate_dockyard_at(
    ...
    if z_max > z_min && x_max > x_min {
        mesh::wave_surface_tris(
            ...
            0.5,
            0.5,
            ((x_max - x_min) / 5.0) as usize,
            ((z_max - z_min) / 5.0).max(1.0) as usize,
            WATER_COLOR,
        );
    }
    ...
    for i in 0..(6.0 * density_scale) as usize {
        ...
        if near_river(wx, wz, river_segments, ww.max(wd) * 0.5 + 2.0) {
            continue;
        }
...
fn generate_interactibles(
    ...
    let car_segs: Vec<&RoadSegment> = net
        .segments
        .iter()
        .filter(|s| s.tier == RoadTier::CarRoad)
        .collect();
    ...
    for i in 0..(4.0 * density_scale) as usize {
        ...
        interactibles.push(Interactible {
            ...
            y,
            z,
            kind: InteractibleKind::PhoneBooth,
            cooldown: 0.0,
            state_val: 0.0,
        });
    }
    ...
    for i in 0..(20.0 * density_scale) as usize {
        ...
        interactibles.push(Interactible {
            ...
            y,
            z,
            kind: InteractibleKind::VendingMachine,
            cooldown: 0.0,
            state_val: 0.0,
        });
    }
    ...
    for i in 0..(8.0 * density_scale) as usize {
        ...
        interactibles.push(Interactible {
            ...
            y,
            z,
            kind: InteractibleKind::ParkBench,
            cooldown: 0.0,
            state_val: 0.0,
        });
    }
    ...
    for i in 0..(4.0 * density_scale) as usize {
        ...
        interactibles.push(Interactible {
            ...
            y,
            z,
            kind: InteractibleKind::NewspaperStand,
            cooldown: 0.0,
            state_val: 0.0,
        });
    }
    ...
    for i in 0..(6.0 * density_scale) as usize {
        ...
        interactibles.push(Interactible {
            ...
            y,
            z,
            kind: InteractibleKind::FireHydrant,
            cooldown: 0.0,
            state_val: 0.0,
        });
    }
    ...
    for i in 0..(2.0 * density_scale) as usize {
        ...
        interactibles.push(Interactible {
            ...
            y,
            z,
            kind: InteractibleKind::Payphone,
            cooldown: 0.0,
            state_val: 0.0,
        });
    }
...
fn generate_river(
    ...
    let bank_heights: Vec<(f32, f32)> = river_segments
        .iter()
        .map(|seg| {
    ...
    if !river_segments.is_empty() {
        let x_min = river_segments
            .iter()
            .map(|s| s.x1.min(s.x2))
            .fold(f32::MAX, f32::min)
            - RIVER_WIDTH * 0.5;
        let x_max = river_segments
            .iter()
            .map(|s| s.x1.max(s.x2))
            .fold(f32::MIN, f32::max)
            + RIVER_WIDTH * 0.5;
        // Average bank height
        let avg_bank_h: f32 = bank_heights
            .iter()
            .map(|(h0, h1)| (h0 + h1) * 0.5)
            .sum::<f32>()
            / bank_heights.len().max(1) as f32;
        ...
        for (bsi, seg) in river_segments.iter().enumerate() {
            ...
            for &side in &[-1.0_f32, 1.0_f32] {
                ...
                let pebble_seed_base =
                    (bsi as u64).wrapping_mul(48611) ^ if side > 0.0 { 99991 } else { 0 };
...
fn generate_bridges(
    ...
    for seg in &net.segments {
        ...
        let rail_start_l = [
            rail_x_l - dir_x * bridge_len * 0.5,
            deck_y + 0.8,
            rail_z_l - dir_z * bridge_len * 0.5,
        ];
        let rail_end_l = [
            rail_x_l + dir_x * bridge_len * 0.5,
            deck_y + 0.8,
        ...
        let rail_start_r = [
            rail_x_r - dir_x * bridge_len * 0.5,
            deck_y + 0.8,
            rail_z_r - dir_z * bridge_len * 0.5,
        ];
        let rail_end_r = [
            rail_x_r + dir_x * bridge_len * 0.5,
            deck_y + 0.8,
...
fn generate_parking_lots(
    ...
    for ni in ring1_start..ring1_end {
        ...
        for attempt in 0..8 {
            ...
            if near_river(cx, cz, river_segments, 16.0) {
                continue;
            }
        ...
        for (ci, corner) in [(-1.0f32, -1.0f32), (1.0, -1.0)].iter().enumerate() {
            ...
            for lci in 0..2 {
                ...
                mesh::leaf_canopy_tris(
                    tris,
                    lcx,
                    tgy + 3.5 + lci as f32 * 0.3,
                    lcz,
                    0.9,
                    40,
                    0.07,
                    (ni * 200 + ci * 10 + lci) as u64,
                    &LOT_LEAF_COLORS,
                );
            }
            trees.push(Tree {
        ...
        street_lights.push(StreetLight {
            x: lx,
            z: lz,
            ground_y: lgy,
        });
...
fn generate_market_stalls(
    ...
    for i in 0..stall_count {
        ...
        // Skip if overlapping buildings
        if overlaps_building(sx, sz, 2.0, buildings) {
            continue;
        }
        if on_road_surface(sx, sz, net) {
            continue;
        }
...
fn generate_bus_stops(
    ...
    for i in 0..stop_count {
        ...
        let Some((perp_x, perp_z, _dir_x, _dir_z, _len)) = segment_perp(seg) else {
            continue;
        };
        ...
        // Skip if overlapping buildings
        if overlaps_building(bx, bz, 2.0, buildings) {
            continue;
        }
...
fn generate_decorations(
    ...
    let town_pos = |rng: &mut Rng| -> (f32, f32) {
        ...
        (
            settlement_center[0] + angle.cos() * radius,
            settlement_center[1] + angle.sin() * radius,
        )
    };
    ...
    for _ in 0..(25.0 * density_scale) as usize {
        ...
        if on_any_road(bx, bz, net) {
            continue;
        }
        if overlaps_building(bx, bz, 0.5, buildings) {
            continue;
        }
        let gy = terrain.height_at(bx, bz);
    ...
    for _ in 0..(12.0 * density_scale) as usize {
        ...
        if on_any_road(px, pz, net) {
            continue;
        }
        if overlaps_building(px, pz, 0.5, buildings) {
            continue;
        }
    ...
    for _ in 0..(5.0 * density_scale) as usize {
        ...
        if overlaps_building(px, pz, 1.5, buildings) {
            continue;
        }
        let gy = terrain.height_at(px, pz);
        // Table top
    ...
    for _ in 0..(3.0 * density_scale) as usize {
        ...
        if overlaps_building(bx, bz, 1.0, buildings) {
            continue;
        }
        let gy = terrain.height_at(bx, bz);
        // Cylinder post
    ...
    for _ in 0..(25.0 * density_scale) as usize {
        let (cx, cz) = town_pos(rng);
        if on_any_road(cx, cz, net) {
            continue;
        }
        if overlaps_building(cx, cz, 0.5, buildings) {
            continue;
        }
        let gy = terrain.height_at(cx, cz);
    ...
    for _ in 0..(20.0 * density_scale) as usize {
        ...
        if on_any_road(sx, sz, net) {
            continue;
        }
        if overlaps_building(sx, sz, 0.5, buildings) {
            continue;
        }
        let gy = terrain.height_at(sx, sz);
    ...
    for _ in 0..(3.0 * density_scale) as usize {
        ...
        if overlaps_building(cx, cz, 1.5, buildings) {
            continue;
        }
        let gy = terrain.height_at(cx, cz);
        ...
        mesh::cylinder_between(
            tris,
            [cx + 0.9, gy + 0.55, cz - 0.3],
            [cx + 1.6, gy + 0.8, cz - 0.3],
            0.025,
            4,
            0xFF664422,
        );
...
fn generate_suburbs(
    ...
    for seg in &net.segments {
        ...
        let dist_from_center = ((mid_x - settlement_center[0]).powi(2)
            + (mid_z - settlement_center[1]).powi(2))
        .sqrt();
        // Only place suburbs in outer ring, away from dockyard
        if dist_from_center < 50.0 || dist_from_center > 170.0 {
            continue;
        }
        if mid_z > WORLD_HALF - 30.0 {
            continue;
        }
        ...
        for side in [-1.0f32, 1.0] {
            ...
            for k in 0..houses_per_side {
                ...
                if near_river(hx, hz, river_segments, 8.0) {
                    continue;
                }

                // Check building overlap
                if overlaps_building(hx, hz, 6.0, buildings) {
                    continue;
                }
                ...
                mesh::box_tris(
                    tris,
                    hx + chim_ox,
                    gy + hh + roof_peak * 0.6 + roof_peak * 0.4 + 0.3,
                    hz + chim_oz,
                    0.45,
                    0.08,
                    0.45,
                ...
                for (wi_idx, wi) in [-1.0f32, 1.0].iter().enumerate() {
                    ...
                    let wh = (k as u32)
                        ...
                        .wrapping_add(wi_idx as u32 * 1013904223);
                ...
                for (si_idx, si) in [-1.0f32, 1.0].iter().enumerate() {
                    ...
                    let sy = gy + hh * 0.55;
                    let sh = (k as u32)
                        .wrapping_mul(741103597)
                        .wrapping_add((si_idx as u32).wrapping_mul(2246822519));
                    let sc = if sh % 100 < 60 {
                        0x00997722
                    } else {
                        SUBURB_WIN_DARK
...
pub fn generate_world(game: &mut GameState) {
    ...
    generate_river(
        &mut game.terrain,
        &mut tris,
        &mut game.world.river_segments,
        seed,
    );

    // === Layer 3: Zone map from terrain analysis ===
    let zone_map = zone::ZoneMap::generate(&game.terrain, &game.world.river_segments, seed);
    let settlement_center = zone_map.main_settlement();
    eprintln!(
        "{} settlements, main at ({:.0}, {:.0}), {} zones classified",
        ...
        zone_map
            .cells
            .iter()
            .filter(|c| c.kind != zone::ZoneKind::Wilderness)
            .count()
    );
    ...
    for (bi, spot) in building_spots.iter().enumerate() {
        if n_arch > 0 && gltf_count < max_gltf_buildings {
            ...
            let rot = (rot_choice as f32) * std::f32::consts::FRAC_PI_4 + rng.range(-0.15, 0.15); // slight jitter
            ...
            emit_gltf_building_scaled(
                &mut tris,
                &mut game.world.buildings,
                &entry.tris,
                spot.x,
                gy,
                spot.z,
    ...
    for (ti, spot) in tree_spots.iter().enumerate() {
        if n_trees > 0 {
            ...
            emit_gltf_tree_scaled(
                &mut tris,
                &mut game.world.trees,
                &entry.tris,
                spot.x,
                gy,
                spot.z,
    ...
    let d = zone_map
        .zone_at(settlement_center[0], settlement_center[1])
        .density
        .max(0.5);
    ...
    if let Some(dock_center) = zone_map.find_zone_center(zone::ZoneKind::Industrial) {
        generate_dockyard_at(
            &mut tris,
            &game.terrain,
            &mut rng,
            &mut game.world.buildings,
            &mut game.world.interactibles,
            &game.world.river_segments,
            dock_center,
            d,
        );
    ...
    for i in 0..NUM_NPCS {
        ...
        let (park_x, park_y, park_z, park_rot, spot_idx) = if spot_offset < total_spots {
            ...
            (
                spot.x,
                game.terrain.height_at(spot.x, spot.z) + VEHICLE_GROUND_OFFSET,
                spot.z,
                spot.rot_y,
                Some(spot_offset),
            )
            ...
            if !game.world.buildings.is_empty() {
                ...
                (
                    px,
                    game.terrain.height_at(px, pz) + VEHICLE_GROUND_OFFSET,
                    pz,
                    0.0,
                    None,
                )
        ...
        game.world.vehicles.push(Vehicle {
            ...
            cruise_speed,
            target_speed: 0.0,
            parking_target: spot_idx,
            parked: true,
            idle_timer: 0.0,
            terrain_normal: crate::math::clamp_normal_tilt(
                game.terrain.normal_at(park_x, park_z),
                30.0,
            ),
            scale,
            body: phys_body,
            wheels: phys_wheels,
            suspension: phys_susp,
            drivetrain: phys_drive,
    ...
    for _ in 0..NUM_ITEMS {
        ...
        loop {
            ...
            if check_walk_collision(&game.world, x, z, 0.5, Some(usize::MAX)) {
                continue;
            }
            if near_river(x, z, &game.world.river_segments, 2.0) {
                continue;
            }
            break;
        }
    ...
    eprintln!(
        "Walk grid: {}x{} ({}m cells)",
        game.walk_grid.grid_w, game.walk_grid.grid_h, game.walk_grid.cell_size
    );
    ...
    eprintln!(
        "World: {} tris, {} road segs ({} nodes), {} buildings, {} trees, {} rocks, {} lights, {} vehicles, {} npcs, {} items, {} bins, {} parking",
        tris.len(),
        game.road_network.segments.len(),
        game.road_network.nodes.len(),
        game.world.buildings.len(),
        game.world.trees.len(),
        game.world.rocks.len(),
        game.world.street_lights.len(),
        game.world.vehicles.len(),
        game.world.npcs.len(),
        game.world.items.len(),
        game.world.trash_bins.len(),
    ...
    for seg in &net.segments {
        match seg.tier {
            RoadTier::CarRoad => {
                ...
                generate_road_strip(
                    tris, terrain, seg.x0, seg.z0, seg.x1, seg.z1, hw, 0.05, ROAD_COLOR,
                );
                generate_road_strip(
                    tris,
                    terrain,
                ...
                if let Some((perp_x, perp_z, _dir_x, _dir_z, _len)) = segment_perp(seg) {
                    ...
                    generate_road_strip(
                        tris, terrain, cl_x0, cl_z0, cl_x1, cl_z1, curb_hw, 0.18, CURB_COLOR,
                    );
                    ...
                    generate_road_strip(
                        tris, terrain, cr_x0, cr_z0, cr_x1, cr_z1, curb_hw, 0.18, CURB_COLOR,
                    );
...
fn emit_building(
    ...
    const WIN_LIT_COLORS: [u32; 4] = [0x00AA7722, 0x00BB8833, 0x00997722, 0x00CCAA44];
    ...
    let front_colors: Vec<u32> = win_holes
        .iter()
        .enumerate()
        .map(|(i, _)| {
            let h = (bi as u32)
            ...
            if h % 100 < 65 {
                WIN_LIT_COLORS[(h / 100) as usize % WIN_LIT_COLORS.len()]
            } else {
                WIN_DARK
            }
        })
    ...
    let back_colors: Vec<u32> = win_holes
        .iter()
        .enumerate()
        .map(|(i, _)| {
            let h = (bi as u32)
                .wrapping_mul(1597334677)
                .wrapping_add(i as u32 * 741103597);
            if h % 100 < 65 {
                WIN_LIT_COLORS[(h / 100) as usize % WIN_LIT_COLORS.len()]
            } else {
                WIN_DARK
    ...
    let side_l_colors: Vec<u32> = side_holes
        .iter()
        .enumerate()
        .map(|(i, _)| {
            let h = (bi as u32)
                .wrapping_mul(3266489917)
                .wrapping_add(i as u32 * 668265263);
            if h % 100 < 65 {
                WIN_LIT_COLORS[(h / 100) as usize % WIN_LIT_COLORS.len()]
            } else {
                WIN_DARK
        ...
        .collect();
    let side_r_colors: Vec<u32> = side_holes
        .iter()
        .enumerate()
        .map(|(i, _)| {
            let h = (bi as u32)
                .wrapping_mul(2246822519)
                .wrapping_add(i as u32 * 387276957);
            if h % 100 < 65 {
                WIN_LIT_COLORS[(h / 100) as usize % WIN_LIT_COLORS.len()]
            } else {
                WIN_DARK
    ...
    if has_shop {
        ...
        mesh::box_tris(
            tris,
            x,
            ground_y + shop_h + 0.05,
            shop_front_z + 0.6,
            w - 0.3,
            0.02,
            1.1,
            darken(awning_color, 0.6),
        );
...
fn emit_tree(
    ...
    const LEAF_COLORS: [u32; 8] = [
        0xFF38883A, 0xFF44AA44, 0xFF2D8830, 0xFF55BB55, 0xFF3A9A3A, 0xFF48B048, 0xFF2A7A2C,
        0xFF40A840,
    ];
    ...
    mesh::leaf_canopy_tris(
        tris,
        x,
        canopy_base_y + canopy_r * 0.1,
        z,
        central_cr,
        60 + (ti * 5) % 20,
        central_cr * 0.2,
        tree_seed.wrapping_add(9999),
        &LEAF_COLORS,
    );
    ...
    for ci in 0..num_canopies {
        ...
        mesh::leaf_canopy_tris(
            ...
            leaves_per_cluster,
            leaf_sz,
            tree_seed.wrapping_add(ci as u64 * 3571),
            &LEAF_COLORS,
        );
    }
    ...
    trees.push(Tree {
        x,
        z,
        trunk_radius: trunk_r + 0.1,
    });
}

fn emit_bush(tris: &mut Vec<WorldTri>, terrain: &Terrain, x: f32, z: f32, bi: usize, seed: u64) {
...
fn emit_grass(
    ...
    const GRASS_COLORS: [u32; 8] = [
        0xFF55BB55, 0xFF66CC55, 0xFF44AA44, 0xFF5DBB4D, 0xFF77CC66, 0xFF55AA55, 0xFF6BC060,
        0xFF88DD77,
    ];
    let gy = terrain.height_at(x, z);
    ...
    mesh::grass_patch_tris(
        ...
        gi as u64 * 4877 + seed,
        &GRASS_COLORS,
        Some(&|gx, gz| terrain.height_at(gx, gz)),
    );
}
...
fn emit_street_light(
    ...
    for pi in 0..8u32 {
        ...
        mesh::push_tri(
            ...
            [
                pool_x + a1.cos() * pool_r,
                pool_y,
                pool_z + a1.sin() * pool_r,
            ],
            [
                pool_x + a0.cos() * pool_r,
                pool_y,
                pool_z + a0.sin() * pool_r,
            ],
            [0.0, 1.0, 0.0],
            pool_color,
        );
...
pub fn check_walk_collision(
    world: &WorldData,
    x: f32,
    z: f32,
    radius: f32,
    home_idx: Option<usize>,
) -> bool {
    for (bi, b) in world.buildings.iter().enumerate() {
        if home_idx == Some(bi) {
            continue;
        }
        if x + radius > b.x - b.w * 0.5
            && x - radius < b.x + b.w * 0.5
            && z + radius > b.z - b.d * 0.5
            && z - radius < b.z + b.d * 0.5
        {
            return true;
        }
