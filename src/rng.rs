...
// All game randomness flows through this to ensure reproducibility from a seed
...
impl Rng {
    ...
    pub fn range(&mut self, min: f32, max: f32) -> f32 { min + self.f32() * (max - min) }

    pub fn pick<T: Copy>(&mut self, arr: &[T]) -> T { arr[self.next() as usize % arr.len()] }
