...
pub fn cylinder_between(
    ...
    for i in 0..n {
        ...
        let offset0 = [
            r * (c0 * right[0] + s0 * fwd[0]),
            r * (c0 * right[1] + s0 * fwd[1]),
            r * (c0 * right[2] + s0 * fwd[2]),
        ];
        let offset1 = [
            r * (c1 * right[0] + s1 * fwd[0]),
            r * (c1 * right[1] + s1 * fwd[1]),
            r * (c1 * right[2] + s1 * fwd[2]),
        ];
...
pub fn wall_with_holes_tris(
    ...
    let v = |x: f32, y: f32, z: f32| -> [f32; 3] { if swap_xz { [z, y, x] } else { [x, y, z] } };
    ...
    for yi in 0..y_cuts.len() - 1 {
        ...
        if (strip_top - strip_bot) < 0.001 {
            continue;
        }

        let strip_holes: Vec<&(usize, &WallHole)> = sorted_holes
            .iter()
        ...
        for (_, hole) in &strip_holes {
            if hole.x > x_start + 0.001 {
                emit_wall_quad(
                    tris, pos_x, pos_y, fz, x_start, strip_bot, hole.x, strip_top, wall_color,
                    face_dir, left_dir, swap_xz,
                );
            }
        ...
        if x_start < wall_w - 0.001 {
            emit_wall_quad(
                tris, pos_x, pos_y, fz, x_start, strip_bot, wall_w, strip_top, wall_color,
                face_dir, left_dir, swap_xz,
            );
        }
    }
    ...
    // Overall wall side edges (left, right, top, back face for solid areas)
    {
        ...
        for yi in 0..y_cuts.len() - 1 {
            ...
            for (_, hole) in &strip_holes {
                if hole.x > x_start + 0.001 {
                    emit_wall_quad(
                        tris, pos_x, pos_y, bz, x_start, strip_bot, hole.x, strip_top, wall_color,
                        -face_dir, left_dir, swap_xz,
                    );
                }
                x_start = hole.x + hole.w;
            ...
            if x_start < wall_w - 0.001 {
                emit_wall_quad(
                    tris, pos_x, pos_y, bz, x_start, strip_bot, wall_w, strip_top, wall_color,
                    -face_dir, left_dir, swap_xz,
                );
            }
        }
...
pub fn wave_surface_tris(
    ...
    for iz in 0..sz {
        for ix in 0..sx {
            ...
            let y00 = base_y
                + (x0 * freq).sin() * (z0 * freq * 0.7).cos() * amplitude
                + (x0 * freq * 2.3 + z0 * freq * 1.7).sin() * amplitude * 0.3;
            let y10 = base_y
                + (x1 * freq).sin() * (z0 * freq * 0.7).cos() * amplitude
                + (x1 * freq * 2.3 + z0 * freq * 1.7).sin() * amplitude * 0.3;
            let y01 = base_y
                + (x0 * freq).sin() * (z1 * freq * 0.7).cos() * amplitude
                + (x0 * freq * 2.3 + z1 * freq * 1.7).sin() * amplitude * 0.3;
            let y11 = base_y
                + (x1 * freq).sin() * (z1 * freq * 0.7).cos() * amplitude
...
pub fn leaf_canopy_tris(
    ...
    for _ in 0..leaf_count {
        ...
        let world_up = if outward[1].abs() < 0.99 {
            [0.0, 1.0, 0.0]
        } else {
            [1.0, 0.0, 0.0]
        };
...
pub fn grass_patch_tris(
    ...
    for _ in 0..blade_count {
        ...
        let by = if let Some(hf) = height_fn {
            hf(bx, bz)
        } else {
            cy
        };
...
pub fn bush_tris(
    ...
    for ci in 0..cluster_count {
        let angle =
            (ci as f32 / cluster_count as f32) * std::f32::consts::TAU + next_f(&mut h) * 0.5;
...
pub fn tapered_cylinder_between(
    ...
    for i in 0..n {
        ...
        let (s1, c1) = (a1.sin(), a1.cos());
        let bot0 = [
            p0[0] + r0 * (right[0] * c0 + fwd[0] * s0),
            p0[1] + r0 * (right[1] * c0 + fwd[1] * s0),
            p0[2] + r0 * (right[2] * c0 + fwd[2] * s0),
        ];
        let bot1 = [
            p0[0] + r0 * (right[0] * c1 + fwd[0] * s1),
            p0[1] + r0 * (right[1] * c1 + fwd[1] * s1),
            p0[2] + r0 * (right[2] * c1 + fwd[2] * s1),
        ];
        let top0 = [
            p1[0] + r1 * (right[0] * c0 + fwd[0] * s0),
            p1[1] + r1 * (right[1] * c0 + fwd[1] * s0),
            p1[2] + r1 * (right[2] * c0 + fwd[2] * s0),
        ];
        let top1 = [
            p1[0] + r1 * (right[0] * c1 + fwd[0] * s1),
            p1[1] + r1 * (right[1] * c1 + fwd[1] * s1),
            p1[2] + r1 * (right[2] * c1 + fwd[2] * s1),
        ];
