// Player job acceptance, tracking, and completion

use crate::state::*;

/// Update player job progress (called each fixed tick)
pub fn sys_player_job(game: &mut GameState, _dt: f32) {
    if game.player.active_job.job_type == PlayerJobType::None { return; }
...
pub fn sys_interactibles_update(world: &mut WorldData, dt: f32) {
    for inter in &mut world.interactibles {
        if inter.cooldown > 0.0 { inter.cooldown -= dt; }
        if inter.state_val > 0.0 { inter.state_val -= dt; }
    }
}
