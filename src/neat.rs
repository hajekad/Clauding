...
pub enum NodeKind { Input, Output, Hidden }
...
impl Genome {
    fn new_minimal(rng: &mut Rng, innovation: &mut InnovationTracker) -> Self {
        ...
        for i in 0..NUM_OUTPUTS {
            nodes.push(NodeGene { id: NUM_INPUTS + i, kind: NodeKind::Output });
        }
        ...
        Genome { nodes, connections, fitness: 0.0, species_id: 0, adjusted_fitness: 0.0 }
    }
...
impl InnovationTracker {
    pub fn new() -> Self {
        InnovationTracker { counter: 0, history: Vec::new() }
    }
    ...
    pub fn next(&mut self, in_node: u16, out_node: u16) -> u32 {
        ...
        for &(i, o, innov) in &self.history {
            if i == in_node && o == out_node { return innov; }
        }
...
impl Population {
    ...
    pub fn evolve(&mut self, fitnesses: &[f32]) {
        ...
        for (i, g) in self.genomes.iter_mut().enumerate() {
            g.fitness = if i < fitnesses.len() { fitnesses[i] } else { 0.0 };
        }
        ...
        let total_adj: f32 = self.genomes.iter().map(|g| g.adjusted_fitness.max(0.0)).sum();
        ...
        for sp in &self.species {
            ...
            let sp_adj: f32 = sp.members.iter()
        ...
        while total_allocated > pop_size {
            ...
            if let Some(max_idx) = offspring_counts.iter().enumerate()
                .filter(|&(_, c)| *c > 1)
                .max_by_key(|&(_, c)| *c)
                .map(|(i, _)| i)
            {
        ...
        for (si, sp) in self.species.iter().enumerate() {
            ...
            sorted_members.sort_by(|&a, &b|
                self.genomes[b].fitness.partial_cmp(&self.genomes[a].fitness).unwrap_or(std::cmp::Ordering::Equal)
            );

            // Elites
            let num_elites = ((count as f32 * ELITE_FRACTION).ceil() as usize).min(sorted_members.len()).max(1);
            ...
            for _ in 0..remaining {
                if new_genomes.len() >= pop_size { break; }

                let use_crossover = self.rng.f32() < CROSSOVER_FRACTION && sorted_members.len() >= 2;
    ...
    fn speciate(&mut self) {
        ...
        for sp in &mut self.species {
            ...
            sp.best_fitness = sp.members.iter()
...
fn compatibility_distance(g1: &Genome, g2: &Genome) -> f32 {
    ...
    if c1.is_empty() && c2.is_empty() { return 0.0; }
    ...
    let w_avg = if matching > 0 { weight_diff_sum / matching as f32 } else { 0.0 };
...
fn crossover(parent1: &Genome, parent2: &Genome, rng: &mut Rng) -> Genome {
    ...
    for c in &child_connections {
        if !node_ids.contains(&c.in_node) { node_ids.push(c.in_node); }
        if !node_ids.contains(&c.out_node) { node_ids.push(c.out_node); }
    }
    // Ensure all input/output nodes exist
    for id in 0..NUM_INPUTS + NUM_OUTPUTS {
        if !node_ids.contains(&id) { node_ids.push(id); }
    }
    node_ids.sort();

    let nodes: Vec<NodeGene> = node_ids.iter().map(|&id| {
        // Find node in parent1 first, then parent2
        parent1.nodes.iter().find(|n| n.id == id)
            .or_else(|| parent2.nodes.iter().find(|n| n.id == id))
            .copied()
            .unwrap_or(NodeGene { id, kind: NodeKind::Hidden })
    }).collect();
...
fn mutate_add_connection(genome: &mut Genome, rng: &mut Rng, innovation: &mut InnovationTracker) {
    ...
    for _ in 0..20 {
        ...
        if to.kind == NodeKind::Input { continue; }
        if from.kind == NodeKind::Output && to.kind == NodeKind::Output { continue; }
        // Check if connection already exists
        let exists = genome.connections.iter().any(|c| c.in_node == from.id && c.out_node == to.id);
        if exists { continue; }
...
fn mutate_add_node(genome: &mut Genome, rng: &mut Rng, innovation: &mut InnovationTracker) {
    let enabled: Vec<usize> = genome.connections.iter().enumerate()
    ...
    genome.nodes.push(NodeGene { id: new_id, kind: NodeKind::Hidden });
...
pub fn execute_outputs(
    ...
    if do_deposit && world.npcs[i].carrying_item {
        ...
        for (bi, bin) in world.trash_bins.iter().enumerate() {
            ...
            let d = dx * dx + dz * dz;
            if d < best_d { best_d = d; best_bi = Some(bi); }
        }
